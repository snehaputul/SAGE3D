...
def gpt_get_score(question, question_type, answer_model, answer_label,openaigpt, model="gpt-3.5-turbo"):
    message=[
    ...
    gpt_response = openaigpt.safe_chat_complete(message, content_only=False) 
    gpt_response = gpt_response.choices[0].message.content
    print(gpt_response)

    while gpt_response is None:
        print('gpt_response is none')
        gpt_response = openaigpt.safe_chat_complete(message, content_only=False) 
        gpt_response = gpt_response.choices[0].message.content
...
def gpt_get_average_score(question, question_type, answer_model, answer_label, openaigpt, model="gpt-3.5-turbo", times=5):
    scores = []
    while len(scores) < times:
        score = gpt_get_score(question, question_type, answer_model, answer_label,openaigpt, model)
        if is_valid(score):
