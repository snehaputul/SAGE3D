...
PROMPT_LISTS = [
    ...
    "Caption this 3D model in 1 sentence."
]
...
def generate_outputs(model, tokenizer, input_ids, point_clouds, stopping_criteria, do_sample=True, temperature=1.0, top_k=50, max_length=2048, top_p=0.95):
    model.eval() 
    # input_ids, point_clouds = input_ids.to(model.dtype), point_clouds.to(model.dtype)
    with torch.inference_mode():
        output_ids = model.generate(
            input_ids,
            point_clouds=point_clouds.to(model.dtype),
            do_sample=do_sample,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--LGA_dim", type=list, default=[1, 1, 2])

    args = parser.parse_args()

    args.LGA_dim = [int(dim) for dim in args.LGA_dim]
    # * check prompt index
    ...
    print(f"[INFO] Running evaluation for {args.task_type} task with prompt index {args.prompt_index}.")
    main(args)